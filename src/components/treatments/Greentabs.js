...
export default function Greentabs({ data }) {
  if (!data?.greentabs) return null;
  ...
  return (
    <main className="page-spacing">
      <section className="page-container px-4 md:px-6 lg:px-10">
        {/* Main Heading */}
        <div className="mb-10">
          <h2 className="text-[32px] md:text-[42px] font-bold text-black text-center font-serif leading-tight">
            {data.greentabs.heading}
          </h2>
        </div>

        {/* Grid Container */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {data.greentabs.items.map((item, index) => (
            <div
              key={index}
