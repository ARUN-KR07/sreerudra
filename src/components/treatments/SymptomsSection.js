  ...
  return (
    <section className="page-spacing">
      <div className="page-container max-w-[1718px] mx-auto">
        {/* Section Heading */}
        <h2 className="text-center text-[36px] md:text-[42px] font-bold text-[#004E22] mb-12 font-serif">
          ...
          {data.symptoms.cards.map((card, index) => (
            ...
            >
              ...
              <div className="flex-grow flex flex-col">
                <h3 className="text-white text-[18px] md:text-[20px] font-bold mb-6 font-serif leading-tight">
                  {card.title}
                </h3>

                {card.description && (
                  <p className="text-gray-200 text-[18px] md:text-[20px] leading-relaxed">
                    {card.description}
                  </p>
                )}
                ...
                {card.list && (
                  <ul className="space-y-4">
                    {card.list.map((item, i) => (
                      <li key={i} className="text-gray-200 text-[18px] md:text-[20px] flex items-start gap-3">
